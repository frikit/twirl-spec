...
/** Compares the same view rendered in several languages.
  ...
  */
trait I18nChecks extends TwirlSpecDsl {
  ...
  /** Every language renders the same page as the base language, differing only
    * in words.
    */
  def translateConsistently: Matcher[Seq[Page]] = matcher(None, Set.empty)

  /** As `translateConsistently`, without the named rules. */
  def translateConsistentlyExcept(ruleIds: String*): Matcher[Seq[Page]] =
    matcher(None, ruleIds.toSet)

  /** Measure the other languages against this one rather than against the first
    * page given.
    */
  ...
  private def compare(
    ...
    val rules = TranslationStandards
      ...
      .filterNot(r => excluded.contains(r.id))
    basePage(pages, base) match {
      case None       => Nil
      case Some(from) =>
        pages
          .filterNot(_ eq from)
          .flatMap(to => rules.flatMap(_.check(from, to)))
    }
  }

  private def basePage(pages: Seq[Page], base: Option[Lang]): Option[Page] =
    base
      .flatMap(l => pages.find(_.lang.code == l.code))
  ...
  private def matcher(
    ...
    new Matcher[Seq[Page]] {
      def apply(pages: Seq[Page]): MatchResult = pages.headOption match {
        ...
        case Some(first) =>
          val report =
            CheckReport(first, compare(pages, base, excluded), "translation")
          MatchResult(
            report.passed,
