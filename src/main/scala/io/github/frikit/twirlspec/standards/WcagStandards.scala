...
object WcagStandards {
  ...
  /** This rule set as one expectation. */
  def expectation(rules: Seq[Rule] = all): Expectation = new Expectation {
    def description: String               = s"accessibility standards (${rules.size} rules)"
  ...
  private def rules: Seq[Rule] = Seq(
    ...
    Rule("title-present", "a page has a non-empty <title>", pageLevel = true) { page =>
      if (page.title.nonEmpty) Nil else Seq(Violation("title-present", "the page has an empty <title>"))
    },
    Rule("html-lang", "the <html> element declares the rendered language", pageLevel = true) { page =>
      page.htmlLang match {
        case None                                     =>
    ...
    Rule("main-landmark", "a page has a <main> landmark", pageLevel = true, Warning) { page =>
      if (page.main.nonEmpty) Nil
      else Seq(Violation("main-landmark", "the page has no <main> landmark").warn.withHint("WCAG 1.3.1"))
    },
    Rule("heading-order", "heading levels are not skipped") { page =>
    ...
    Rule("labelled-controls", "every form control has an accessible name") { page =>
    ...
    Rule("grouped-choices", "radios and checkboxes sit in a fieldset with a legend") { page =>
    ...
    Rule("submit-has-name", "the submit control has visible text") { page =>
      page.document
    ...
    Rule("table-header-scope", "table headers declare a scope", severity = Warning) { page =>
      page.document
    ...
    Rule("link-has-name", "every link has an accessible name") { page =>
      page.links.elements
    ...
    Rule("link-text-is-meaningful", "link text makes sense out of context", severity = Warning) { page =>
      page.links.texts
    ...
    Rule("new-tab-is-announced", "links opening a new tab say so", severity = Warning) { page =>
      page.links.elements
    ...
    Rule("image-alt", "every image has an alt attribute") { page =>
