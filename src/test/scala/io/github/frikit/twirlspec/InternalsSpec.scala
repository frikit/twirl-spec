...
class InternalsSpec extends AnyWordSpec with Matchers with TwirlSpec {
  ...
  "failure messages with nothing to show" should {
    ...
    "say so rather than print an empty string" in {
      ...
      renderedMessage(empty, heading(literal("Something")))                                          must include("(empty)")
      renderedMessage(empty, errorSummaryContaining("x", "kitchenSink.error.email.required"))        must
        include("errorSummary")
      renderedMessage(pageOf("<h1>A</h1>"), summaryList("checkAnswers.name" -> "Ada"))               must
        include("(no summary list rows)")
      renderedMessage(pageOf("<h1>A</h1>"), tableHeaders("kitchenSink.col1"))                        must include("(no table headers)")
      renderedMessage(pageOf("<h1>A</h1>"), tableRow("Ada"))                                         must include("(no table rows)")
      renderedMessage(pageOf("<h1>A</h1>"), link("kitchenSink.link"))                                must include("(no links on the page)")
      renderedMessage(pageOf("<h1>A</h1>"), summaryRow("checkAnswers.name"))                         must
        include("(no summary list on the page)")
      renderedMessage(pageOf("<h1>A</h1>"), fieldError("email", "kitchenSink.error.email.required")) must
    ...
    "name the fields that do have errors" in {
      ...
      renderedMessage(page, fieldError("b", "kitchenSink.error.email.required")) must include("errors on: a")
    }
    ...
    "report a checkbox selection mismatch both ways round" in {
      ...
      renderedMessage(page, checkboxGroup("c").nothingSelected) must include("(nothing selected)")
      val unselected = pageOf(
        """<h1>A</h1><fieldset><legend>Colours</legend>
          |<input type="checkbox" id="c" name="c" value="red"><label for="c">Red</label></fieldset>""".stripMargin
      )
      renderedMessage(unselected, checkboxGroup("c").selectedIs("red")) must include("(nothing selected)")
    }
  }
  ...
  "the standards" should {
    ...
    "flag an empty title, a missing main landmark and a nameless submit control" in {
      ...
      val fired   = allStandardsExpectation.check(noTitle).map(_.rule)
    ...
    "quote the surrounding text when a Scala value leaks in" in {
      ...
      val v    = allStandardsExpectation.check(leak).find(_.rule == "no-scala-leakage")
  ...
  private def renderedMessage(page: Page, e: io.github.frikit.twirlspec.expect.Expectation): String =
