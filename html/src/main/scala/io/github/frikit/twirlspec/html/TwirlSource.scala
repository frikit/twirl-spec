...
/** A Twirl template with the Scala taken out, so that what is left is the HTML
  * it writes.
  *
  * Scanning a template rather than a rendered page is cheap — no application,
  * no injector, no render — but the source is two languages at once, and one of
  * them uses the angle brackets the other reserves for tags.
  * `@if(page < total)` is a comparison, not the start of an element, and a
  * scanner that cannot tell the difference reports every paginated page in a
  * large codebase.
  *
  * Everything belonging to an `@` expression is blanked to spaces, keeping
  * newlines so line numbers still point at the template. The `{ … }` block that
  * follows a construct like `@if` or `@formHelper` is left alone, because that
  * part is HTML.
  */
object TwirlSource {
  ...
  /** `} else if (…) {` continues an `@if` without an at-sign of its own, so the
    * condition is Scala even though nothing marks it as such. Only a closing
    * brace introduces one, which keeps the word "else" in ordinary prose out of
    * it.
    */
  private def blankElseContinuation(
      src: String,
      out: Array[Char],
      at: Int
  ): Int = {
    ...
    if (
      !src.startsWith("else", elseAt) || isIdentPart(
        charAtOr(src, elseAt + 4, ' ')
      )
    ) at + 1
    else {
      ...
      if (
        !src.startsWith("if", ifAt) || isIdentPart(charAtOr(src, ifAt + 2, ' '))
      ) ifAt
      else {
        blank(out, ifAt, ifAt + 2)
        val paren = skipWhitespace(src, ifAt + 2)
        if (charAtOr(src, paren, ' ') == '(')
          blankBalanced(src, out, paren, '(', ')')
        else paren
      }
    }
  }
  ...
  private def blankExpression(src: String, out: Array[Char], at: Int): Int =
    if (at + 1 >= src.length) {
      ...
      src.charAt(at + 1) match {
        ...
        case '{' =>
          blank(out, at, at + 1); blankBalanced(src, out, at + 1, '{', '}')
        case '(' =>
          blank(out, at, at + 1); blankBalanced(src, out, at + 1, '(', ')')
  ...
  /** `@name`, and whatever continues it — but not a following block, which is
    * HTML.
    */
  ...
  /** `@name = { … }` names a piece of markup for use elsewhere. Half a tag pair
    * is the whole point of one, so its contents are not the balance check's
    * business.
    */
  private def isFragmentDefinition(src: String, from: Int): Boolean = {
    val eq = skipWhitespace(src, from)
    charAtOr(src, eq, ' ') == '=' && charAtOr(
      src,
      skipWhitespace(src, eq + 1),
      ' '
    ) == '{'
  }

  private def blankFragmentDefinition(
      src: String,
      out: Array[Char],
      from: Int
  ): Int = {
  ...
  /** Argument lists, type parameters and `.field` chains all belong to the
    * expression.
    */
  private def blankContinuations(
      src: String,
      out: Array[Char],
      from: Int
  ): Int = {
    ...
    while (continue)
      if (j < src.length && src.charAt(j) == '(')
        j = blankBalanced(src, out, j, '(', ')')
      else if (j < src.length && src.charAt(j) == '[')
        j = blankBalanced(src, out, j, '[', ']')
      else if (
        j + 1 < src.length && src
          .charAt(j) == '.' && isIdentStart(src.charAt(j + 1))
      ) {
        val start = j
        j += 1
        while (j < src.length && isIdentPart(src.charAt(j))) j += 1
  ...
  /** From an opening bracket to the one that matches it, ignoring brackets
    * inside strings.
    */
  private def blankBalanced(
      src: String,
      out: Array[Char],
      start: Int,
      open: Char,
