...
class TwirlSpecApiSpec extends AnyWordSpec with Matchers with Bilingual {
  ...
  "a violation" should {
    ...
    "truncate very long values" in {
      val long = "x" * 500
      io.github.frikit.twirlspec.expect.Violation
        .mismatch("rule", long, long)
        .render() must include("...")
    }
  }
  ...
  "normalised and messageText" should {
    ...
    "make a message comparable with page text" in {
      ...
      messageText("whatIsYourName.title") mustBe normalised(
        messages("whatIsYourName.title")
      )
    }

    "resolve arguments before normalising" in {
      messageText("whatIsYourName.heading") mustBe normalised(
