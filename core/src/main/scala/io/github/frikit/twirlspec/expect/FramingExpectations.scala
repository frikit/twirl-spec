...
/** Expectations about the frame of a GOV.UK page: what it is called, what it is
  * headed, and the furniture the layout is responsible for.
  */
trait FramingExpectations {
  ...
  /** The browser title, ignoring the " - Service name - GOV.UK" suffix the
    ...
    */
  def title(key: String, args: Any*): Expectation = title(
  ...
  def title(expected: Expected): Expectation = Expectation("title") { page =>
    compare(
      "title",
      expected,
      stripErrorPrefix(page, page.title),
      page,
      StartsWith
    )
  }
  ...
  def exactTitle(literal: String): Expectation = Expectation("exactTitle") {
    page =>
      compare("exactTitle", Expected.Literal(literal), page.title, page, Exact)
  }

  /** The single `<h1>`. */
  def heading(key: String, args: Any*): Expectation = heading(
    Expected.Key(key, args.toSeq)
  )

  /** The `<h1>`, given as the exact words rather than a message key. */
  def headingText(literal: String): Expectation = heading(
  ...
  def heading(expected: Expected): Expectation = Expectation("heading") {
    page =>
      ...
      if (h1.isEmpty) Seq(Violation.missing("heading", "h1"))
        ...
        val headingText =
          if (captionText.nonEmpty && h1.text.startsWith(captionText))
            h1.text.drop(captionText.length).trim
          else h1.text
  ...
  def caption(key: String, args: Any*): Expectation = caption(
    Expected.Key(key, args.toSeq)
  )

  /** The caption, given as the exact words rather than a message key. */
  def captionText(literal: String): Expectation = caption(
    Expected.Literal(literal)
  )
  ...
  def caption(expected: Expected): Expectation = Expectation("caption") {
    page =>
      present("caption", page.caption) match {
        case Nil =>
          compare("caption", expected, page.caption.text, page, Contains)
        case errs => errs
      }
  }

  /** The service or site name in the header. */
  def serviceName(key: String = "service.name"): Expectation =
    Expectation("serviceName") { page =>
      Matching.exactlyOne("serviceName", page.serviceName) match {
        case Left(v)  => Seq(v)
        case Right(e) =>
          compare("serviceName", Expected.Key(key), e.text(), page, Exact)
      }
    }
  ...
  val noBackLink: Expectation =
    Expectation("noBackLink")(page => absent("noBackLink", page.backLink))
  ...
  def subheading(key: String, args: Any*): Expectation =
    headingAtLevel(2, Expected.Key(key, args.toSeq))
          ...
          if (found.exists(Text.same(_, value))) Nil
            ...
            Seq(
              Violation(
                ...
                expected = Some(value),
                actual = Some(
                  if (found.isEmpty) "(no h" + level + " elements)"
                  else found.mkString(" | ")
                )
              )
            )
          ...
          if (actual == url) Nil
            ...
            Seq(
              Violation.mismatch(
                ...
                url,
                actual,
                "back link pointed somewhere else"
              )
            )
        }
    }
