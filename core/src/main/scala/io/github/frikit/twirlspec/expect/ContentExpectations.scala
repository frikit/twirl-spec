...
trait ContentExpectations {
  ...
  def bullets(keys: String*): Expectation = listItems("bullets", ".govuk-list--bullet > li", keys.toList)

  /** The numbered list contains exactly these items, in order. */
  def numberedItems(keys: String*): Expectation = listItems("numberedItems", ".govuk-list--number > li", keys.toList)

  private def listItems(rule: String, selector: String, keys: List[String]): Expectation =
    Expectation(rule) { page =>
      val resolved   = keys.map(k => (k, Expected.Key(k).resolve(page)))
      val unresolved = resolved.collect { case (_, Left(v)) => v.copy(rule = rule) }
          ...
          Seq(
            Violation
              .mismatch(
                rule,
                expected.mkString(" | "),
                if (actual.isEmpty) "(no list items matched)" else actual.mkString(" | ")
              )
              .at(selector)
          )
      }
  ...
  def summaryList(rows: (String, String)*): Expectation = Expectation("summaryList") { page =>
    val resolved   = rows.toList.map { case (k, v) => (Expected.Key(k).resolve(page), v) }
    val unresolved = resolved.collect { case (Left(v), _) => v.copy(rule = "summaryList") }
    if (unresolved.nonEmpty) unresolved
    else {
      val expected = resolved.collect { case (Right(k), v) => (k, Text.normalise(v)) }
      ...
      if (actual == expected) Nil
        ...
        Seq(
          Violation.mismatch(
            ...
            if (actual.isEmpty) "(no summary list rows)" else actual.map { case (k, v) => s"$k = $v" }.mkString(" | ")
          )
        )
    }
  }
      ...
      if (actual == expected) Nil
        ...
        Seq(
          Violation.mismatch(
            "tableHeaders",
            expected.mkString(" | "),
            if (actual.isEmpty) "(no table headers)" else actual.mkString(" | ")
          )
        )
    }
  }
    ...
    if (rows.contains(expected)) Nil
      ...
      Seq(
        Violation.mismatch(
          "tableRow",
          expected.mkString(" | "),
          if (rows.isEmpty) "(no table rows)" else rows.map(_.mkString(" | ")).mkString("\n                ")
        )
      )
  }
  ...
  private def contentIn(rule: String, extract: Page => String, expected: Expected): Expectation =
    Expectation(rule) { page =>
  ...
  private def componentText(rule: String, select: Page => Selection, expected: Expected): Expectation =
    Expectation(rule) { page =>
      val sel = select(page)
...
final case class LinkExpectation(text: Expected, href: Option[String], id: Option[String]) extends Expectation {

  def to(url: String): LinkExpectation                 = copy(href = Some(url))
  def withId(elementId: String): LinkExpectation       = copy(id = Some(elementId))
  def saying(key: String, args: Any*): LinkExpectation = copy(text = Expected.Key(key, args.toSeq))
        ...
        if (candidates.isEmpty)
          Seq(
            Violation(
              ...
              expected = Some(if (expectedText.nonEmpty) expectedText else id.map("#" + _).getOrElse("")),
              actual = Some(
                if (page.links.isEmpty) "(no links on the page)"
                else page.links.texts.filter(_.nonEmpty).take(12).mkString(" | ")
              )
            )
          )
        else {
          val textIssues =
            ...
            else Matching.compare(rule, text, candidates.text, page, Matching.Exact)

          val hrefIssues = href.toSeq.flatMap { url =>
            val actual = candidates.attrs("href")
            if (actual.contains(url)) Nil
            else Seq(Violation.mismatch(s"$rule href", url, actual.mkString(", ")))
          }
...
final case class SummaryRowExpectation(
  ...
  def withChangeLinkTo(url: String): SummaryRowExpectation = copy(changeHref = Some(url))
