...
class QualityStandardsSpec extends AnyWordSpec with Matchers with TwirlSpec {
  ...
  private val allRules =
    Rule.expectation(SemanticStandards.all ++ PerformanceStandards.all ++ MetadataStandards.all)

  private def page(body: String, head: String = """<meta charset="utf-8">"""): Page =
    Page.fromString(
  ...
  private def fired(body: String, head: String = """<meta charset="utf-8">"""): Seq[String] =
    allRules.check(page(body, head)).map(_.rule)

  private def find(body: String, rule: String, head: String = """<meta charset="utf-8">""") =
    allRules.check(page(body, head)).find(_.rule == rule)

  "semantics" should {

    "reject a control inside another control" in {
      fired("""<a href="/x">Read <button>more</button></a>""")     must contain("no-nested-interactive")
      fired("""<button>Save <a href="/x">now</a></button>""")      must contain("no-nested-interactive")
      fired("""<a href="/x">Read more</a><button>Save</button>""") must not contain "no-nested-interactive"
    }

    "reject a non-list-item inside a list" in {
      fired("<ul><li>One</li><div>Two</div></ul>")             must contain("lists-contain-list-items")
      fired("<ul><li>One</li><li>Two</li></ul>")               must not contain "lists-contain-list-items"
      fired("<ul><li>One</li><script>var x = 1</script></ul>") must not contain "lists-contain-list-items"
    }
  ...
  "page weight" should {
    ...
    "warn about an image with no dimensions" in {
      fired("""<img src="/logo.png" alt="Logo">""")                        must contain("images-have-dimensions")
    ...
    "warn about a render-blocking script, and accept defer, async or module" in {
      fired("<p>x</p>", head = """<meta charset="utf-8"><script src="/a.js"></script>""") must
        contain("scripts-are-deferred")
      Seq("defer", "async", """type="module"""").foreach { attr =>
        fired("<p>x</p>", head = s"""<meta charset="utf-8"><script src="/a.js" $attr></script>""") must
    ...
    "warn about an oversized data uri and an oversized style block" in {
      val big = "A" * 11000
      fired(s"""<img src="data:image/png;base64,$big" alt="Big">""")             must contain("no-oversized-data-uri")
      fired("<p>x</p>", head = s"""<meta charset="utf-8"><style>$big</style>""") must
        contain("no-large-inline-style")
      fired("""<img src="data:image/png;base64,AAAA" alt="Small">""")            must not contain "no-oversized-data-uri"
    }
  }
  ...
  "metadata" should {
    ...
    "require a character encoding" in {
      ...
      fired("<p>x</p>", head = """<meta http-equiv="Content-Type" content="text/html; charset=utf-8">""") must
        not contain "has-charset"
    }

    "warn about noindex, a missing description and an overlong title" in {
      fired("<p>x</p>", head = """<meta charset="utf-8"><meta name="robots" content="noindex, nofollow">""") must
        contain("not-noindex")
      fired("<p>x</p>")                                                                                      must contain("has-meta-description")
      fired("<p>x</p>", head = """<meta charset="utf-8"><meta name="description" content="A page">""")       must
        not contain "has-meta-description"
      fired("<p>x</p>", head = """<meta charset="utf-8"><meta name="description" content="   ">""")          must
    ...
    "skip page level rules for a fragment" in {
      allRules.check(Page.fromString("<p>just a component</p>", english, messages)).map(_.rule) must
        not contain "has-charset"
    }
  }
  ...
  "the module" should {
    "expose its rules with unique ids, disjoint from the other sets" in {
      val ours = (SemanticStandards.all ++ PerformanceStandards.all ++ MetadataStandards.all).map(_.id)
      ours.distinct.size                                               mustBe ours.size
      ours                                                               must have size 12
      SemanticStandards.only("no-br-for-layout").map(_.id)             mustBe Seq("no-br-for-layout")
      PerformanceStandards.allExcept("images-have-dimensions").map(_.id) must
        not contain "images-have-dimensions"
      MetadataStandards.expectation().description                        must include("12".take(0) + "rules")
    }
  }
