...
/** Two checks about the spec rather than the page.
  ...
  */
trait CoverageChecks { self: TwirlSpecDsl =>
  ...
  /** Which part of the page a spec answers for. Defaults to the main content, so the layout's header and footer are not the view spec's problem. */
  def coverageScope: String = ContentCoverage.defaultScope

  /** Ids, links or tracking values every page in this project inherits from its layout.
    *
    * Declare them once on the project's spec base rather than in each view spec.
    */
  def coverageIgnored: Set[String] = Set.empty

  /** Every id, link and tracked element on the page was asserted by some test in this spec.
    *
    * Put it in the last test of the spec: it can only see the assertions that have already run.
    */
  def assertEverything: Matcher[Page] = assertEverythingExcept()

  /** As `assertEverything`, but these ids, links or tracking values are deliberately not asserted. */
  ...
  /** What this spec has asserted so far, for a spec that wants to report rather than fail. */
  def unassertedContent(page: Page): List[ContentCoverage.Anchor] =
    ContentCoverage.unasserted(page, coverageIgnored, trackedAttributes, coverageScope)
