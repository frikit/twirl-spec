...
object MetadataStandards extends RuleSet {
  ...
  lazy val all: Seq[Rule] = Seq(
    Rule(
      "has-charset",
      "the page declares its character encoding",
      pageLevel = true
    ) { page =>
      if (
        page.document
          .select("meta[charset], meta[http-equiv=Content-Type]")
          .asScala
          .nonEmpty
      ) Nil
      else
        Seq(
          Violation("has-charset", "the page declares no character encoding")
            .withHint(
              """<meta charset="utf-8"> first in the head, or a browser guesses"""
            )
        )
    },
    Rule(
      "not-noindex",
      "the page is not accidentally hidden from search",
      pageLevel = true,
      Warning
    ) { page =>
      page.document
        ...
        .map(e =>
          Violation(
            ...
            actual = Some(e.attr("content"))
          ).warn
            .withHint(
              "deliberate on a holding page, and a surprise everywhere else"
            )
        )
    },
    Rule(
      "has-meta-description",
      "the page describes itself for a search result",
      pageLevel = true,
      Warning
    ) { page =>
      val description =
        page.document.select("meta[name=description]").asScala.toList
    ...
    Rule(
      "title-is-concise",
      "the title survives being truncated",
      pageLevel = true,
      Warning
    ) { page =>
      ...
      if (title.length <= TitleBudget) Nil
        ...
        Seq(
          Violation(
            ...
            "a browser tab and a search result both cut it off before the end"
          )
        )
    }
  )
